$(document).ready(function() {
    ...
    oCalendar.fullCalendar({
        ...
        monthNamesShort: ['jan', 'feb', 'maa', 'apr', 'mei',
            'jun', 'jul', 'aug', 'sep', 'okt', 'nov', 'dec'
        ],
        dayNames: ['zondag', 'maandag', 'dinsdag', 'woensdag',
            'donderdag', 'vrijdag', 'zaterdag'
        ],
        dayNamesShort: ['zo', 'ma', 'di', 'wo', 'do', 'vr',
            'za', 'zo'
        ],
        buttonText: {
            today: '<i class=\'icon-home\'></i> vandaag',
            month: 'maand',
            week: 'week',
            day: 'dag'
        },
        eventSources: [{
            url: $('.cottage_booker__action--fetchall')
                .attr('href'),
            type: 'POST',
            allDayDefault: true
        }, {
            url: $(
                '.cottage_booker__action--fetchallexceptions'
            ).attr('href'),
            type: 'POST',
            allDayDefault: true,
            color: 'green'
        ...
        eventClick: function(calEvent, jsEvent, view) {
            if (calEvent.type == 'exception') {
                ...
                $('.cottage_booker__exception').html('<p>' +
                    calEvent.notes + '</p><p>' +
                    calEvent.title + '</p>');
                ...
                if (calEvent.authorized) {
                    populateBookingForm(calEvent.id,
                        calEvent.start, calEvent.end,
                        calEvent.notes, calEvent.persons
                    );
                    setFormType('update');
                    showView('book-form');
                } else {
                    flashMessage(
                        'U kunt deze reservering niet aanpassen!',
                        'error');
                }
            }
        }
    ...
    addCalButton('.cottage_booker__view--full-calendar', 'center',
        'cottage_booker__action--new');
    ...
    // Show the calendar
    $('.cottage_booker__book-form__button--view-calendar').click(
        function(e) {
            e.preventDefault();
...
function createOrUpdate() {
    if (!$('#cottage_booker__book-form__id').val()) {
        ...
        if (!$('#cottage_booker__book-form__start').val()) {
            $('#cottage_booker__book-form__start').parents('.control-group').addClass(
                'error');
            bError = true;
        }
        if (!$('#cottage_booker__book-form__end').val()) {
            $('#cottage_booker__book-form__end').parents('.control-group').addClass(
                'error');
            bError = true;
        }
        if (!$('#cottage_booker__book-form__persons').val()) {
            $('#cottage_booker__book-form__persons').parents('.control-group').addClass(
                'error');
            bError = true;
        }
        if (!bError) {
            var start = $.fullCalendar.parseDate($(
                '#cottage_booker__book-form__start').datepicker(
                'getDate'));
            var end = $.fullCalendar.parseDate($(
                '#cottage_booker__book-form__end').datepicker('getDate'));
            newBooking(start, end, $('#cottage_booker__book-form__notes').val(),
                $('#cottage_booker__book-form__persons').val());
        }
    } else {
        // Update
        var bError = false;
        ...
        if (!bError) {
            ...
            var end = $.fullCalendar.parseDate($(
                '#cottage_booker__book-form__end').datepicker('getDate'));
            updateBooking($('#cottage_booker__book-form__id').val(), start, end,
                $('#cottage_booker__book-form__notes').val(), $(
                    '#cottage_booker__book-form__persons').val());
        }
    }
}
...
function setFormType(sType) {
    if (sType == 'new') {
        ...
        $('.cottage_booker__book-form__button--book').html(
            '<i class=\'icon-ok icon-white\'></i> Reserveer');
        ...
        $('.cottage_booker__book-form__button--book').html(
            '<i class=\'icon-ok icon-white\'></i> Reservering bijwerken');
...
function populateBookingForm(id, start, end, notes, persons) {
    ...
    $('#cottage_booker__book-form__start').datepicker('setDate', $.fullCalendar
        .formatDate(start, 'dd-MM-yyyy'));
    if (!end) {
        $('#cottage_booker__book-form__end').datepicker('setDate', $.fullCalendar
            .formatDate(start, 'dd-MM-yyyy'));
    } else {
        $('#cottage_booker__book-form__end').datepicker('setDate', $.fullCalendar
...
function flashMessage(text, type) {
    type = type || 'success';
    $('.alert').alert('close');
    $('.cottage_booker__flashmessages').prepend('<div class=\'alert alert-' +
        type +
...
function updateBooking(id, start, end, notes, persons) {
    ...
    $.get(url, {
        ...
        if (data.status !== 'error') {
            showView('full-calendar');
            $('.cottage_booker__view--full-calendar').fullCalendar(
                'refetchEvents');
        }
    });
}
...
function addCalButton(parent, where, className) {
    ...
    var myButton =
        ...
        text + '</span>' + '<span class=\'fc-header-space\'></span>';
    $('.' + className).html(myButton);
    $('.' + className).show();
}
...
function updateBookingCredits() {
    if (!$('#cottage_booker__book-form__start').val()) {
        ...
        $('#cottage_booker__book-form__end').parents('.control-group').removeClass(
            'warning');
        $('#cottage_booker__book-form__start').parents('.control-group').removeClass(
            'warning');
        var url = $('.cottage_booker__action--credits').attr('href');
        var start = $.fullCalendar.parseDate($(
            '#cottage_booker__book-form__start').datepicker('getDate'));
        var end = $.fullCalendar.parseDate($('#cottage_booker__book-form__end')
            .datepicker('getDate'));
        $.get(url, {
            start: $.fullCalendar.formatDate(start, 'yyyy-MM-dd'),
            end: $.fullCalendar.formatDate(end, 'yyyy-MM-dd')
